  ...
  try {
    ...
    if (existing.status !== 'review' && existing.status !== 'draft') {
      return NextResponse.json(
        {
          error: `Cannot reject a passage in status '${existing.status}'. Only review and draft passages can be rejected.`,
        },
        { status: 400 },
      );
    }
    ...
    logError(error, 'api/teacher/reading/passages/reject');

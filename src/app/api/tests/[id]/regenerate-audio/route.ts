...
import { regenerateTestItemAudio } from '@/lib/practice/test-audio';
...
export async function POST(
  ...
  let body: { itemId?: unknown; voiceId?: unknown };
  try {
  ...
  try {
    const result = await regenerateTestItemAudio(id, body.itemId, voiceId);
    if (!result) {
      return NextResponse.json({ error: 'Item not found or has no audio' }, { status: 404 });
    }
    return NextResponse.json({ itemId: body.itemId, ...result });
  } catch (error) {
    logError(error, 'tests.regenerateAudio');

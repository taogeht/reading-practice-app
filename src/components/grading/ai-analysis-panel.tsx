...
// Custom ease-out curve used on every meaningful state change. The
// built-in CSS easings are too soft for product UI; this one feels
// intentional without bouncing.
const EASE_OUT = 'cubic-bezier(0.23, 1, 0.32, 1)';

// Mandarin pairs with English at equal size and weight throughout —
// never as a translation subtitle. The native stack keeps glyph
// proportions correct.
const ZH = 'font-["PingFang_TC","Noto_Sans_TC","Microsoft_JhengHei",sans-serif]';

// System serif reserved for prose surfaces (teacher_summary, prosody
// notes, strengths/focus items, passage, transcript) where reading
// authority matters. UI labels and metric numerals stay sans.
const SERIF = 'font-serif';

// Single accent (rose-700, a deep madder) carries achievement and
// urgency. Amber-700 is a quieter warning. Everything else is stone.
...
// Prosody score (1–4) as a small horizontal scale with a tick at the
// position — reads as a calibrated measurement rather than a count.
  ...
  return (
    <div className="grid grid-cols-[6rem_1fr_2.25rem] items-center gap-4 py-1.5">
      ...
      <div className="relative h-px bg-stone-200">
        <span
          className="absolute top-1/2 -translate-y-1/2 -translate-x-1/2 w-px h-3 bg-stone-900 transition-[left] duration-300"
          style={{ left: `${pct}%`, transitionTimingFunction: EASE_OUT }}
          aria-hidden
        />
...
// Each expanded-panel block fades in with a small upward shift,
// staggered ~40ms behind the previous block. Builds a sense of
// cascade rather than dump.
function sectionStyle(order: number): React.CSSProperties {
  return {
    animation: `section-in 300ms ${EASE_OUT} both`,
  ...
  return (
    <div className="mt-4 border border-stone-200 bg-stone-50/50">
      <button
        ...
        className="w-full flex items-center justify-between gap-4 px-4 py-2.5 text-left transition-[background-color] duration-150 hover:bg-stone-100/60 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-rose-600/40 focus-visible:ring-inset"
      >
                ...
                <>
                  ...
                  {wpmScore !== null && (
                    <>
                      <span className="text-stone-300">·</span>
                      <span>
                        <span className="tabular-nums">{wpmScore}</span>
                        <span className="text-stone-500"> wpm</span>
                      </span>
                    </>
                  )}
                </>
        ...
        {/* Single rotating chevron with the project ease-out curve.
            Replaces the dual icon swap which couldn't animate. */}
        <ChevronDown
      ...
      {expanded && (
        <div
          className="px-5 pt-6 pb-8 space-y-9 bg-white border-t border-stone-200"
          style={{ animation: `panel-fade 200ms ${EASE_OUT} both` }}
        >
          {hasResults && analysisJson && (
            <>
              {/* TEACHER'S READ — paired EN + ZH prose, equal size +
                  weight, hairline column rule. Leads the panel because
                  it's the human-readable headline. */}
              {teacherSummary && (
              ...
              {/* METRICS — five cells separated by 1px verticals.
                  Numerals are sans tabular at text-[28px] — large enough
                  to read as the data spine of the panel, restrained
                  enough to stay product-appropriate. */}
              <section className="border-y border-stone-200" style={sectionStyle(1)}>
                <div className="grid grid-cols-2 md:grid-cols-5 md:divide-x divide-stone-200">
                  <div className="px-4 py-4">
                    <div className="text-[10px] uppercase tracking-[0.14em] text-stone-500 mb-1.5">Grade</div>
                    <div className="flex items-baseline gap-2">
                      <span className={`w-1.5 h-1.5 rounded-full ${gradeTone(letterGrade)}`} />
                      <span className="text-[28px] font-medium text-stone-900 tabular-nums leading-none">
                        {letterGrade ?? '—'}
                      </span>
                    </div>
                  </div>
                  {hasFluency ? (
                    <>
                      <div className="px-4 py-4">
                        ...
                        <div className="text-[28px] font-medium text-stone-900 tabular-nums leading-none">
                          {wcpm}
                        </div>
                      </div>
                      <div className="px-4 py-4">
                    ...
                    <>
                      ...
                      <div className="px-4 py-4 col-span-2 md:col-span-3 flex items-center">
                        <span className="text-[11px] text-stone-500">
                          Pre-Phase-7 recording. Re-analyze for the full fluency profile.
                        </span>
                      </div>
                    </>
                  )}
              ...
              {analysisJson.hallucinationSuspected && (
                <div className="flex items-start gap-2 text-[13px] text-amber-800" style={sectionStyle(2)}>
              ...
              {/* STRENGTHS + FOCUS — two columns, Roman-numeral gutter,
                  EN + ZH at parity beneath each item. No tinted card
                  backgrounds; the single rose / amber dot carries the
                  semantic. */}
              {(analysisJson.claude?.prosody?.strengths?.length ||
                analysisJson.claude?.prosody?.focusAreas?.length) ? (
                <section
                  className="grid grid-cols-1 md:grid-cols-2 md:divide-x divide-stone-200 gap-y-8"
              ...
              {/* PASSAGE — single column of serif prose with
              ...
              {/* TRANSCRIPT — serif italic, indented from the column
                  edge by a 1px rule. Reads as a quoted utterance. */}
              {transcript && (
                <section style={sectionStyle(6)}>
              ...
              {/* TALLY — quiet caption row at the bottom. */}
              <div
                className="flex flex-wrap items-baseline gap-x-4 text-[11px] text-stone-500 pt-2 border-t border-stone-200"
          ...
          <div className="flex justify-end pt-2">
            <Button
              ...
              className="h-auto px-0 gap-1.5 text-[12px] text-stone-600 hover:text-stone-900 hover:bg-transparent active:scale-[0.97] transition-transform duration-150 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-rose-600/40 focus-visible:ring-offset-2"
              style={{ transitionTimingFunction: EASE_OUT }}
            >
...
// 1..N to Roman numerals for the strengths / focus lists. Caps at 20
// defensively though we never expect more than ~5 items.

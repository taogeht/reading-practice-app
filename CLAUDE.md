...
- **Image generation is pluggable** — all callers import the `imageClient` facade from `src/lib/image` (NOT the provider clients directly). The facade routes to whichever backend the `image.generationModel` admin setting names: **Google Gemini Flash** (`gemini-client.ts`, model `gemini-2.5-flash-image`, default) or **OpenAI GPT Image 1 mini** (`gpt-image-client.ts`, model `gpt-image-1-mini`, quality `low` — the cheap option for quality testing). Both implement the shared `ImageClient` interface in `types.ts` and share prompt templates from `prompts.ts`. Covers spelling word images, reading-passage panels, base character portraits, scene backgrounds, cosmetic items. The setting lives in System Settings (`/admin/settings`); `IMAGE_GEN_MODEL` env is a fallback. Note: `imageClient.isConfigured()` is **async** (it resolves the provider first) — `await` it.
